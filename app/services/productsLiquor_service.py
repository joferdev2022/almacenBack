...
from app.db.mongo import products_liquorDb
from ..utils.helpers_liquor import product_helper
...
async def retrieve_products(page: int, xpage: int, local:int):
    ...
    for product in products_liquorDb.find({"local": local}).skip(skipProducts).limit(xpage):
...
async def add_product(product_data: dict) -> dict:
    # Remove null _id that comes from jsonable_encoder
    product_data.pop("_id", None)
    product_data.pop("id", None)

    product_data["_id"] = ObjectId()

    if not product_data.get("fechaDeCreacion"):
        product_data["fechaDeCreacion"] = datetime.now()

    product = products_liquorDb.insert_one(product_data)
    new_product = products_liquorDb.find_one({"_id": product.inserted_id})
...
async def delete_product_by_id(product_id: str):
    ...
    result =  products_liquorDb.delete_one(filter)
...
async def upload_excel(file_data: bytes, local: int):
    ...
    for record in records:
        ...
        filter_query = {
            ...
            "presentacion": {"$regex": f"^{presentacion_normalizada}$", "$options": "i"},
            "local": local
        }
        
        result = products_liquorDb.update_one(
            filter_query,
            {"$set": mapped_record},
            upsert=True
        )
...
async def download_excel(local: int):
    ...
    productos = list(products_liquorDb.find({"local": local}))
    ...
    for prod in productos:
        datos.append({
            ...
            "P. VENTA": (prod.get("sku", [{}])[0].get("precioVenta", 0) if prod.get("sku") else 0),
         })
